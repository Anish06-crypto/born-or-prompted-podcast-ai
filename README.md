...
## KTP Research Alignment

This project was built as empirical groundwork for a Knowledge Transfer Partnership (KTP) application. The experiment framework directly addresses the KTP's core research objectives:

**Innovation challenge**: AI-generated content at scale requires agents that are distinguishable, consistent, and commercially deployable — yet the relationship between model selection, prompt engineering, and emergent voice identity is poorly understood.

**Research contribution**:
- First systematic comparison of persona consistency across 7 LLMs spanning 8B–235B parameters on a standardised corpus
- Isolates model effects (Exp A), prompt effects (Exp B), and their interaction (Exp C) using a three-study factorial design
- Provides quantitative metrics that can be used as KPIs in a production content pipeline

**Commercial pathway**: Findings directly inform model and prompt selection decisions for AI content platforms — reducing trial-and-error engineering costs and providing an evidence base for scaling persona-driven AI products.

**Reproducibility**: All conditions are deterministic (fixed temperature, seed topics, checkpoint-resumable runner), enabling replication and extension by academic partners.
